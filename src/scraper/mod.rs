...
impl Scraper {
    ...
    /// are deduplicated based on their normalized URL to ensure each item appears only once.
    /// URLs are normalized by removing query parameters (e.g., `?_pos=1&_sid=abc`) to
    /// prevent the same product from appearing multiple times with different search tracking.
    ///
    /// # ID Generation
    ///
    /// Each jacket gets a unique ID generated by taking the MD5 hash of its normalized product URL
    /// and formatting it as a hexadecimal string. This ensures consistent identification
    /// across scraping sessions and search terms, regardless of query parameters.
    ///
    /// # Image URL Handling
    ///
